...
const CharacterForm = ({ initial, onSave, onCancel, isSaving }: CharacterFormProps) => {
  ...
  const [systemPrompt, setSystemPrompt] = useState(
    initial?.systemPrompt ?? DEFAULT_SYSTEM_PROMPT,
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="mb-1.5 block text-sm font-medium">グリーティング（会話開始時のメッセージ）</label>
        <textarea
          value={greeting}
...
export const CharacterManager = ({ onCharacterSelect }: CharacterManagerProps) => {
  ...
  const { characters, isLoading, createCharacterEntry, updateCharacterEntry, deleteCharacterEntry } =
    useCharacterQuery();
  ...
  const renderAvatar = (character: Character) => {
    ...
    if (av.startsWith("http")) {
      return (
        <img src={av} alt={character.name} className="h-8 w-8 rounded-full object-cover" />
      );
    }
    return av;
  };
